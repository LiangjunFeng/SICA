...
if __name__ == '__main__': 
    ...
    data = pd.read_excel('/Users/zhuxiaoxiansheng/Desktop/sensorreading.xlsx')

    label = data['label']
    data = data.drop('label',axis=1)
    
    traindata = data.iloc[0:int(0.95*data.shape[0]),:]
    testdata = data.iloc[int(0.95*data.shape[0]):,:]
    
    trainlabel = label.iloc[0:int(0.95*data.shape[0])]
    testlabel = label.iloc[int(0.95*data.shape[0]):]
